...
export default function WorkspaceBoard({ id }: { id: string }) {
  ...
  return (
    ...
    >
      <div className="h-screen bg-slate-950 text-white flex flex-col">
        <div className="px-6 pt-6 pb-2 flex items-center gap-4">
          <button
            ...
            className="px-4 py-2 text-sm font-medium rounded-lg border border-blue-500/30 text-blue-400 hover:border-blue-400 hover:bg-slate-800 transition-all active:scale-95"
          >
            ‹ Back
          </button>
          <h1 className="text-xl font-semibold text-slate-200">Board</h1>
        </div>
        ...
        {!dataLoaded ? (
          <div className="flex gap-4 p-6 overflow-hidden">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="w-80 shrink-0 rounded-xl p-4 bg-slate-900 border border-slate-700/50"
              >
          ...
          <div className="flex gap-4 overflow-x-auto p-6 flex-1 stagger">
            {lists.map((list, i) => (
        ...
        {/* Floating add button */}
        <button
          className="w-14 h-14 text-4xl rounded-full fixed right-6 bottom-6 bg-blue-600 border border-blue-400 text-white hover:bg-blue-500 hover:shadow-[0_0_24px_rgba(59,130,246,0.6)] transition-all duration-200 active:scale-90 flex items-center justify-center leading-none shadow-[0_0_16px_rgba(59,130,246,0.4)] animate-pulse-glow"
          onClick={() => {
            setOpenModal(true)
            setTitle("")
          }}
      ...
      <DragOverlay>
        {activeTask ? (
          <div className="bg-slate-800 p-3 rounded-lg border border-blue-500 shadow-2xl cursor-grabbing opacity-90 w-72">
